...
NEW_DATABASE_FILE = '80percent.csv'
df = pd.read_csv(DATABASE_FILE)
IMAGE_SIZE = 28


def saveDatabase(newdf):
    newdf.to_csv(NEW_DATABASE_FILE, index=False)

def loadDatabase():
    newdf = pd.read_csv(NEW_DATABASE_FILE)
    return newdf
...
def main_func():
    ...
    for row in range(int(len(df)*0.7), int(len(df)*0.8)):
        try:
